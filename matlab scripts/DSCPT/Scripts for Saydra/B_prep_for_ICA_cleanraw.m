...
for j = 1:length(subIDs)
    ...
    try
     ...
     %% Event codes look reasonable can come back and fix later if necessary
     ...
     %% downsample and filter
        ...
        %[ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 0,'gui','off','setname','resamped and filted ');
        eeglab redraw % this is the command I have been looking for my whole life!!!
     ...
     %% automatic bad elec and bad epoch rejection
        ...
        %[ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 0,'gui','off','setname','auto-rejected');
      ...
      %% save it
        ...
        toc
        %save number of epochs deleted
        %prep_report(j,2) = {round(sum(selectedregions(:,2) - selectedregions(:,1))/250, 2)};
