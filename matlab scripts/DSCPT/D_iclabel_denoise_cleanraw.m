...
%         %% rereference to get Cz elec
         ...
         new_locs = readlocs('/labs/srslab/data_main/VJP_ICA/scripts/montages/BrainVision_128_clabel.elp');
         old_locs = {EEG.chanlocs.labels};
         old_locs(strcmp(old_locs,'R_Ear')) = {'G21'}; %have to change ears to match .elp file
         old_locs(strcmp(old_locs,'L_Ear')) = {'G10'};
         old_locs(end+1) = {'G24'}; % have to add Cz color label

         %% need to re-sort new chanlocs according to old locs
          [~,idx] = ismember({new_locs.labels},old_locs);
          EEG.data = EEG.data(idx,:);
          EEG.chanlocs = new_locs;
          %% this is a nightmare but basically I had to use the color names (e.g. G1) to match the channel order of the
          % elp file in srslab with the channels output 
          % by the brainvision import function. So now they are aligned and
          % we can swap out the color names with the 10-10 names
          new_locs = readlocs('/labs/srslab/data_main/VJP_ICA/scripts/montages/BrainVision_128_10-10.elp');
          EEG.chanlocs = new_locs;
          EEG = pop_reref( EEG,[]);
